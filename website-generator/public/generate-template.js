...
function generateWebsite(topic, siteType, features, colorTheme, imgUrl, vidUrl, menuItems, options) {
  menuItems = menuItems || [];
  options = options || {};
...
${hasGoogleLogin ? `${'<'}script src="https://accounts.google.com/gsi/client" async defer>${'<'}/script>` : ''}
<style>
:root{
...
/* Google login button */
.google-login-btn{display:inline-flex;align-items:center;gap:.45rem;padding:.4rem .9rem .4rem .65rem;
  border-radius:9999px;border:1px solid var(--card-border);background:rgba(255,255,255,.04);
  color:var(--text);font-family:inherit;font-size:.75rem;font-weight:500;cursor:pointer;
  transition:all .3s var(--ease);position:relative;white-space:nowrap}
.google-login-btn:hover{background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.12)}
.google-login-btn.logged-in{border-color:rgba(52,211,153,.2);background:rgba(52,211,153,.06)}
.google-user-menu{position:absolute;top:calc(100% + .5rem);right:0;min-width:200px;
  background:var(--surface);border:1px solid var(--card-border);border-radius:.75rem;
  box-shadow:0 8px 32px rgba(0,0,0,.4);overflow:hidden;z-index:60}
.google-user-menu img{width:28px;height:28px;border-radius:50%;object-fit:cover}
...
@media(max-width:768px){
  ...
  .google-user-menu{position:fixed;top:auto;bottom:2rem;left:1rem;right:1rem}
}
...
<header class="site-nav" id="site-nav">
  <nav>
    ...
    <div class="nav-links" id="nav-links">
      ${navLinks}
      ${hasGoogleLogin ? `<button class="google-login-btn" id="googleLoginBtn" onclick="handleGoogleLogin()">
        ...
        <span id="googleUserName">로그인</span>
      </button>
      <div class="google-user-menu" id="googleUserMenu" style="display:none">
        <div id="googleUserInfo" style="padding:.75rem 1rem;border-bottom:1px solid var(--card-border);font-size:.78rem"></div>
        <button onclick="handleGoogleLogout()" style="width:100%;padding:.6rem 1rem;background:none;border:none;color:var(--text-sec);font-size:.75rem;cursor:pointer;text-align:left;font-family:inherit">로그아웃</button>
      </div>` : ''}
    </div>
  </nav>
</header>
...
${hasGoogleLogin ? `
  ...
  window.handleGoogleLogin = function() {
    if (window._googleUser) {
      const menu = document.getElementById('googleUserMenu');
      menu.style.display = menu.style.display === 'none' ? 'block' : 'none';
      return;
    }
    if (!window.google?.accounts?.id) {
      alert('Google 로그인 SDK 로드 중입니다. 잠시 후 다시 시도해주세요.');
      return;
    }
    google.accounts.id.initialize({
      ...
      callback: function(response) {
        ...
        if (info) info.innerHTML = '<div style="display:flex;align-items:center;gap:.5rem"><img src="' + (payload.picture||'') + '"><div><div style="font-weight:600">' + (payload.name||'') + '</div><div style="font-size:.65rem;color:var(--text-sec)">' + (payload.email||'') + '</div></div></div>';
      }
    });
  ...
  window.handleGoogleLogout = function() {
    ...
    document.getElementById('googleUserMenu').style.display = 'none';
