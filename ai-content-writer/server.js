...
9) URL 슬러그 + 내부링크 추천 문장 + 관련/롱테일 키워드`,
  'naverblog-html': `네이버 블로그 에디터에 그대로 붙여넣을 수 있는, 가독성 높은 '본문 HTML'을 생성하라.
규칙:
- 모든 스타일은 inline style 속성으로만(외부 CSS·class 금지 — 블로그 에디터가 제거함).
- 시각 요소를 풍부하게: 핵심 요약 박스(연한 배경 콜아웃), 강조 포인트 박스, 체크리스트(✅), 단계 카드(번호 원형), 인용구, 형광펜 강조(span background), 구분선(hr), 이모지 소제목, 표(table, 줄무늬 배경).
- 톤: 친근한 후기·정보체, 존댓말, 적당한 이모지.
- 구조: h2/h3 소제목, p 문단, 박스는 div style.
- 이미지 자리: 회색 점선 박스에 '📷 이미지 추천: ...' 안내.
- 색상은 네이버 친화적인 부드러운 톤(예: #03c75a 포인트, 연녹/연회색 배경).
- 전체는 본문 요소만(html/head/body 래퍼 금지).
출력은 HTML 코드만. 설명·마크다운 코드펜스(\`\`\`) 절대 금지.`,
  'googleblog-html': `구글 SEO에 최적화된, 가독성 높은 '본문 HTML'을 생성하라.
규칙:
- 모든 스타일은 inline style 속성으로만(외부 CSS·class 금지).
- 시맨틱·구조 중심: h1 1개, h2/h3 위계, 문단, 목차(앵커 링크 ul), FAQ(질문 강조 + 답변 박스).
- 시각 요소: 정보 콜아웃 박스, 핵심 요약 박스, 비교 표(table), 체크리스트, 단계 카드, 형광펜 강조(span background), 구분선.
- 톤: 전문적·정보 중심, 검색 의도 충족(첫 문단에 핵심 답).
- 이미지 자리: 회색 점선 박스에 '📷 이미지 추천: ...' 안내.
- 색상은 깔끔한 비즈니스 톤(예: #2563eb 포인트, 연청/연회색 배경).
- 전체는 본문 요소만(html/head/body 래퍼 금지).
출력은 HTML 코드만. 설명·마크다운 코드펜스(\`\`\`) 절대 금지.`,
